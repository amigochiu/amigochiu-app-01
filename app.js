...
function getExchangeRateDisplay() {
    ...
    return `1 ${state.fromCurrency} ≈ ${rate.toLocaleString('zh-TW', { maximumFractionDigits: 4 })} ${state.toCurrency}`;
}
...
function render() {
    ...
    <div class="h-[100dvh] w-full transition-colors duration-500 ${currentTheme.bg} flex items-center justify-center font-sans text-slate-800 relative overflow-hidden">
        ...
        <div class="w-full h-full max-w-md relative z-auto flex flex-col">
            ...
            <!-- Header -->
            <div class="p-4 flex justify-between items-center border-b relative z-50 ${currentTheme.divider} shrink-0">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h1 class="font-bold text-2xl sm:text-3xl leading-tight ${currentTheme.textMain}">
                            匯率計算機 <span class="text-[10px] sm:text-xs font-medium ${currentTheme.textSub} ml-1">v2025.12.23.01</span>
                        </h1>
                        <div class="flex items-center gap-1.5 text-xs font-medium ${currentTheme.textSub}">
                            <div class="w-1.5 h-1.5 rounded-full ${state.loading ? 'bg-amber-400 animate-pulse' : 'bg-emerald-400'}"></div>
                            ${state.loading ? '更新中...' : `最後更新：${state.lastUpdated}`}
                        </div>
                    </div>
                </div>
                <div class="flex gap-2 relative">
                    ...
                    <div class="absolute top-full right-0 mt-2 w-48 p-2 rounded-2xl shadow-xl border backdrop-blur-3xl overflow-hidden z-[60] flex flex-col gap-2 ${currentTheme.card}">
                        <div class="px-2 py-1 text-xs font-bold opacity-50 ${currentTheme.textMain}">選擇主題配色</div>
                        ${themeItems}
                    </div>` : ''}

                    <button type="button" onclick="refreshRates()" class="p-2 rounded-xl transition-colors ${currentTheme.refreshBtn}">
                        <i data-lucide="refresh-cw" class="w-[18px] h-[18px] ${state.loading ? 'animate-spin' : ''}"></i>
                    </button>
                </div>
            </div>

            <!-- 內容區 (Flex Grow to fill space) -->
            <div class="relative z-10 flex-1 flex flex-col min-h-0">
                <div class="p-5 pb-2 flex-col flex flex-1 min-h-0 overflow-y-auto">
                    ...
                    <div class="flex flex-col mb-1 relative flex-1 justify-end [container-type:size]">
                        <div class="w-full flex justify-between items-end mb-1">
                            ${CurrencySelector(state.toCurrency, "setToCurrency", "目標", "left", true, currentTheme)}
                    ...
                    <!-- 交換按鈕 (Compact) -->
                    <div class="relative h-8 flex items-center justify-center my-1 shrink-0">
                        ...
                        >
                            <i data-lucide="arrow-up-down" class="w-4 h-4 stroke-[2.5px]"></i>
                        </button>
                    </div>
                    ...
                    <div class="flex flex-col mt-1 mb-2 flex-1 justify-start [container-type:size]">
                        <div class="w-full flex justify-start mb-1">
                            ${CurrencySelector(state.fromCurrency, "setFromCurrency", "持有", "left", false, currentTheme)}
                        </div>
                ...
                <!-- Keypad Area -->
                <div class="p-3 sm:p-5 grid grid-cols-4 gap-2 sm:gap-3 border-t backdrop-blur-sm transition-colors ${currentTheme.keyboardBg} shrink-0">
